...
import { consumeEvent, RabbitMQconnection } from "./utils/rabbitmq.js";
import { handleDeleteFromSearch, handlePostCreateInSearch } from "./eventHandler/search.event.handler.js";
...
async function startServer() {
  try {
    ...
    //consume events
    await consumeEvent('post.created',handlePostCreateInSearch)
    await consumeEvent('post.deleted',handleDeleteFromSearch)
    app.listen(PORT, () => {
      logger.info(`Search Server Running on Port : ${PORT}`);
    });
  } catch (error) {
    logger.error(`Error while stating server : ${error}`)
  }
}
startServer()
